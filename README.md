...
지식: Obsidian vault ⇄ Chroma RAG    스케줄: launchd(트렌드 수집 트리거 전용)
목표: ① 쇼츠 자동화  ② 주식/임베디드는 현재 설계 문서만 유지
```

## 요구 사항

- macOS + Homebrew, Python 3.11+, Claude Code 설치
- Discord 계정과 본인 서버(길드) 1개
- Docker Desktop/n8n은 선택 사항이며 현재 기본 운영 경로는 launchd
...
python scripts/healthcheck.py      # 필수 항목 ✅ 확인
...
# 선택: bash scripts/install_launchd.sh  # 트렌드 수집 스케줄 등록
```
...
| `hermes/` | 큐(bus)·라우터·Ollama 워커·큐 자동 소비자 |
| `channel/` | Discord 봇 + webhook/local log 알림 |
| `memory/` | vault→Chroma 인덱서, RAG 검색 |
| `pipelines/` | 쇼츠 파이프라인 + 트레이딩/임베디드 설계 문서 |
| `vault/` | Obsidian으로 여는 지식 저장소 (ADR·런북·회고) |
| `scripts/` | bootstrap, healthcheck, launchd 설치 |
| `data/` | SQLite·Chroma·토큰·런타임 로그 (git 제외) |
...
1. 오케스트레이션은 HERMES 단일 계층 — n8n/launchd에는 판단 로직 금지
2. 비용 라우팅: 초안·요약·분류는 Ollama/`hermes.worker`, 코드·설계·리뷰는 Claude
3. 외부 영향 작업(배포·게시·실거래 변경)은 전부 Discord 승인 경유
4. **주식 실행·임베디드 실행은 현재 스코프 제외** — 문서만 유지
5. **주문 실행 경로에 LLM 금지** — `pipelines/trading/README.md`
6. 결정은 ADR로 기록하고 인덱싱 — 미래의 HERMES가 검색해 쓴다
